...
export default class App extends React.Component{
  ...
  fetch_weather=()=> {
    fetch('http://api.openweathermap.org/data/2.5/weather?q='+this.state.city+'&appid=038315906b121cd8567d438ab60bc87b') 
      ...
      .then((json) => {
        ...
        this.setState({ min : (json.main.temp_min-273.15).toFixed(2)+"°C"})
      })
      ...
      style={styles.Image_Background_Style}>
        ...
        <View style={styles.Info_Box_View}>
          <View style={styles.Info_Holder_Veiw}>
            ...
            <Text style={styles.humidity_text}>Humedad : {this.state.humidity}</Text>
            <Text style={styles.other_text}>Presión : {this.state.pressure}</Text>
              <Text style={styles.other_text}>Temperatura Max : {this.state.temp_min }</Text>
          </View>
        </View>
      </ImageBackground>
